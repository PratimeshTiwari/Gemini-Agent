...
export function App({ agentLoop, wsServer }) {
  ...
  interactiveTurns.forEach(turn => {
    const { actions } = parseTurnActions(turn);
    if (actions.length > 0) {
      // Turn header accordion
      focusableItems.push({ type: 'turn_actions', id: `turn_${turn.id}`, turnId: turn.id, turn });

      // Individual tool runs, thoughts, and command outputs inside the turn
      actions.forEach(act => {
        focusableItems.push({
          type: act.type,
          id: act.id,
    ...
    if (!isProcessing && history.length > 0) {
      ...
      if (lastMsg && (lastMsg.role === 'assistant' || lastMsg.role === 'agent') && !lastMsg.isLocal) {
        ...
        if (cleanContent && cleanContent !== lastRevealedContent.current) {
          ...
          setRevealedLength(0);
          let charPos = 0;
          // Paced reveal: 1 char for normal responses, 2 for longer ones
          const step = cleanContent.length > 1200 ? 3 : (cleanContent.length > 500 ? 2 : 1);
          const interval = setInterval(() => {
            ...
            <Box key="banner" flexDirection="column" marginBottom={1} width="100%">
              <Box borderStyle="round" borderColor="dim" flexDirection="column" paddingX={2} paddingY={0} width="100%" alignItems="center">
                ...
                <Box flexDirection="column" width="100%">
                  ...
                  <Text color="gray">Extension: <Text color={extensionConnected ? 'green' : 'yellow'}>{extensionConnected ? 'Connected' : 'Waiting for Extension'}</Text></Text>
                </Box>
              </Box>
            </Box>
          );
              ...
              <Box key={turn.id} flexDirection="column" marginBottom={1} width="100%" flexShrink={1}>
                ...
                {actions.length > 0 && (
                  <Box flexDirection="column" width="100%" flexShrink={1}>
                    <Box flexDirection="column" marginBottom={1} width="100%" flexShrink={1}>
                      <Text color={isTurnHeaderFocused ? 'cyan' : 'gray'}>
                        {turnHeaderPrefix}
                        {isTurnActionsExpanded ? '▼' : '▶'} Thinking & Actions {isLastTurn && isProcessingTurn ? <Text color="cyan"><Spinner type="dots" /> {thinkingDisplayText || status}</Text> : <Text dimColor>[{duration}s]</Text>}
                      </Text>

                      {isTurnActionsExpanded && (
                        <Box flexDirection="column" paddingLeft={2} borderLeftStyle="single" borderLeftColor="dim" marginLeft={4} marginTop={1} width="100%" flexShrink={1}>
                          {actions.map((act, idx) => {
                            const isActFocused = !isStatic && focus === FOCUS_CHAT && focusableItems[clampedSelectedToolIdx]?.id === act.id;
                            const isActExpanded = expandedLogIds.has(act.id);
                            const actPrefix = isActFocused ? <Text color="cyan">❯ </Text> : <Text>  </Text>;

                            if (act.type === 'tool') {
                              const statusIcon = act.success === false ? '❌' : (act.result ? '✅' : '⏳');
                              const argsStr = JSON.stringify(act.args || {});
                              const shortArgs = argsStr.length > 50 ? argsStr.substring(0, 50) + '...' : argsStr;

                              return (
                                <Box key={act.id || idx} flexDirection="column" marginY={0}>
                                  <Text color={isActFocused ? 'cyan' : 'gray'}>
                                    {actPrefix}
                                    {isActExpanded ? '▼' : '▶'} {statusIcon} ran {act.toolName} <Text dimColor>{shortArgs}</Text>
                                  </Text>
                                  {isActExpanded && act.result && (
                                    <Box flexDirection="column" paddingLeft={4} marginY={1} width="100%" flexShrink={1}>
                                      <Box borderStyle="round" borderColor={isActFocused ? 'cyan' : 'dim'} paddingX={1} width="100%" flexShrink={1}>
                                        <Text dimColor wrap="wrap">
                                          {(() => {
                                            const resStr = typeof act.result === 'string' ? act.result : JSON.stringify(act.result, null, 2);
                                            const lines = resStr.split('\n');
                                            return lines.length > 25 ? lines.slice(0, 25).join('\n') + '\n... [Result Truncated]' : resStr;
                                          })()}
                                        </Text>
                                      </Box>
                                    </Box>
                            ...
                            if (act.type === 'command_output') {
                              return (
                                <Box key={act.id || idx} borderStyle="round" borderColor={isActFocused ? 'cyan' : 'dim'} paddingX={1} marginTop={1} flexDirection="column" width="100%" flexShrink={1}>
                                  <Text bold color="cyan">{actPrefix}Command Output</Text>
                            ...
                            if (act.type === 'system') {
                              return (
                                <Box key={act.id || idx} marginY={0} width="100%" flexShrink={1}>
                                  <Text dimColor wrap="wrap">{actPrefix}∙ {act.msg.content}</Text>
                                </Box>
                              );
                            }

                            if (act.type === 'image') {
                              return <Text key={act.id || idx} dimColor>{actPrefix}∙ Attached Image: {act.content}</Text>;
                            }
